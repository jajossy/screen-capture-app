...
//const {desktopCapture} = window.require('electron').remote;
...
class Snipper extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            //upload_url : 'http://localhost:8989/upload'
        };
    };

    initCropper(e){
        mainWindow = this.getCurrentWindow();
        //console.log(mainWindow); // comment on console
        mainWindow.hide();
    ...
    getCurrentWindow(){
        //return electronDevtoolsInstaller.remote.getCurrentWindow();
        ...
        setTimeout(() => {
            ...
            this.getScreenShot((base64data) => {
                ...
                Jimp.read(encondedImageBuffer, (err, image) => {
                    ...
                    console.log(screen.getPrimaryDisplay()); //test
                    console.log(screen.getPrimaryDisplay().size.width); //test
                    console.log(screen.getPrimaryDisplay().size.height); //test
                    console.log(screenSize); // test*/
                    let crop = coordinates ?
                                image.crop(coordinates.x, coordinates.y, parseInt(coordinates.width, 10), parseInt(coordinates.height, 10)) :
                                image.crop(0,0, screen.getPrimaryDisplay().size.width, screen.getPrimaryDisplay().size.height);
                    //alert(crop); // test
    ...
    resizeWindowFor(view){
        if(view === 'snip'){
            ...
            let x = (screen.getPrimaryDisplay().size.width/2) - 400;
            let y = (screen.getPrimaryDisplay().size.height/2) - 250;
            ...
            let x = (screen.getPrimaryDisplay().size.width/2) - width / 2;
            let y = (screen.getPrimaryDisplay().size.height/2) - height / 2;
            mainWindow.setPosition(x,y);
        }
    }
                            ...
                            <div>
                                <h2>
                                    <img height="25"
                                        ...
                                        alt="" />
                                        Farouk and Joseph Snipper - Project First Phase
                                </h2>
                            </div>
